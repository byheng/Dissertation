...
在软硬件功能划分方面，本章遵循"逻辑密集型任务归 PS、数据密集型规则计算归 PL"的原则实现深度解耦。PS 端 ARM 处理器负责多传感器数据同步、SVO 2.0 视觉前端追踪（含 FAST 特征提取、光流跟踪与深度滤波）、滑动窗口状态管理（含关键帧选择与边缘化先验更新）以及基于 EKF 框架的轻量化位姿与路标点增量更新；PL 端 FPGA 则专注于观测处理与雅可比并行生成、Hessian 矩阵块在线累加、SOB 编码解析与动态门控，以及 Schur 消元引擎的调度执行。这一划分使得每次后端迭代中计算最密集的雅可比生成与矩阵消元操作完全卸载至硬件流水线，而需要复杂分支判断的状态管理逻辑则保留在处理器端，实现了计算效率与编程灵活性的有机统一。
