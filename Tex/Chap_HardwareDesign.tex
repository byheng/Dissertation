...
为保证加载、计算与写回三个阶段稳定衔接并保持数据流一致性，加速器内部设计了主控制状态机（FSM），其状态转移与流水线执行关系如图~\ref{fig:main_fsm} 所示。FSM 从 \texttt{IDLE} 状态响应启动信号，依次完成参数加载（\texttt{LOAD\_PARAMS}）、观测与位图并行读取（\texttt{LOAD\_INPUTS}），随后进入 Phase2/Phase3 计算循环，并在完成 Schur 更新后通过 DMA 写回结果，最终回到空闲状态等待下一帧。该调度框架以阶段化控制为主线，通过计数器与握手信号精确约束各模块的启动顺序与数据可用性，从而避免乱序访问与资源争用，同时保证 PL 端与 PS 端的数据交换满足因果顺序。
...
针对特征点对应的 $3 \times 3$ 海森矩阵块 $\mathbf{H}_{mm}$，本系统实现了专用的求逆器单元，其计算流程如图~\ref{fig:hmm_inv_flow} 所示，RTL 数据通路如图~\ref{fig:hmm_inv_rtl} 所示。设对称矩阵写为
\begin{equation}
