...
\begin{figure}[h!t]
    \centering
    \resizebox{0.95\linewidth}{!}{\input{Img/software_arch_tikz.tex}}
...
序列 & CPU & FPGA \\
\midrule
MH\_01\_easy & 0.490 & \textbf{0.042} \\
...
\midrule
\textbf{Avg} & \textbf{0.119} & \textbf{0.073} \\
\bottomrule
\end{tabular}
\end{table}
...
表~\ref{tab:resource_comparison} 展示了本文设计与现有工作的 FPGA 资源占用对比。可以看到，本文设计在保持性能的同时，BRAM 占用仅为 \textbf{67~KB}，显著低于同类工作，充分体现了 SOB 编码与紧凑存储在片上资源利用方面的优势。

\begin{table*}[h!t]
\centering
\bicaption{与现有工作的 FPGA 资源占用对比}{Comparison of FPGA resource utilization with existing works}
...
\begin{figure}[h!t]
\centering
\includegraphics[width=0.9\linewidth]{Img/outdoortest}
