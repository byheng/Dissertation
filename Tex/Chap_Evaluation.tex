...
为保证公开数据集评测与真实平台测试的一致性，本文分别整理了 EuRoC、TUM-VI 以及异构计算应用平台（XCZU19EG）所使用的相机与 IMU 标定参数。数据集实验中，系统直接读取数据集对应的标定文件；真实平台实验中，则需要对板载双目相机和 ICM-20948 IMU 进行离线标定，以生成系统运行所需的参数文件。上述参数不仅决定前端去畸变、双目重投影与三角化的精度，也直接影响视觉惯性预积分与后端优化的数值稳定性。
...
相机模型 & pinhole & pinhole \\
畸变模型 & radial-tangential & radial-tangential \\
$f_x$ & 482.5662 & 480.4561 \\
$f_y$ & 482.6128 & 480.5389 \\
$c_x$ & 333.3413 & 331.0532 \\
$c_y$ & 268.2853 & 241.4544 \\
$k_1$ & -0.385194 & -0.388507 \\
$k_2$ & 0.152262 & 0.162251 \\
$p_1$ & 0.000063 & -0.000017 \\
$p_2$ & 0.000245 & -0.000325 \\
$\mathbf{t}_{B\rightarrow C}$ (m) & $[0.00107, -0.01471, 0.04808]^\mathrm{T}$ & $[0.10409, -0.01424, 0.04817]^\mathrm{T}$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\subsection{XCZU19EG 平台传感器标定流程}

为了获得异构计算应用平台运行所需的双目相机标定参数和 IMU 噪声模型参数，本文对 XCZU19EG 平台上的双目相机与 IMU 进行了离线标定。整体流程可分为相机内外参标定、相机--IMU 外参与时间对齐验证以及 IMU 噪声参数标定三个步骤。

首先，在双目相机标定阶段，使用打印的 AprilTag/棋盘格标定板采集多组静态与动态图像序列，尽可能覆盖不同距离、姿态与视场位置，以增强参数估计的可观测性。随后利用双目标定工具同时求解左右相机的焦距、主点、畸变参数以及左右相机相对于机体系的外参矩阵。标定过程中需重点保证标定板在图像中的尺度变化充分、边缘区域覆盖完整，并通过重投影误差剔除质量较差的图像帧。最终得到的标定结果将用于系统在真实平台上的图像去畸变、双目几何约束构建与重投影残差计算，其流程如图~\ref{fig:xczu19eg_calib_flow} 所示。

其次，在相机与 IMU 协同标定阶段，需要在保持硬件同步链路开启的条件下采集包含平移、旋转和俯仰变化的短时激励序列，使视觉与惯性测量在多个运动模态下均具备足够激励。由于本文平台已经通过 PL 侧 FSYNC 实现了图像曝光中心与 IMU 采样时刻的硬件级同步，因此标定重点放在相机坐标系、机体系与惯性测量坐标系之间的空间关系一致性检查上。通过对齐视觉重建轨迹与惯性积分轨迹，可以验证外参估计是否稳定，并检查是否存在时间戳漂移、坐标轴定义不一致或个别传感器安装误差等问题。
...
最后，在 IMU 噪声参数标定阶段，将平台静置采集长时间惯性数据，并采用 Allan 方差分析方法估计陀螺仪与加速度计的噪声密度和随机游走系数。静置数据能够抑制真实运动带来的扰动，使随机噪声特性更加突出。根据 Allan 方差曲线在不同积分时间区间内的斜率特征，可分别提取陀螺仪噪声密度 $\mathrm{gyr}_n$、陀螺仪偏置随机游走 $\mathrm{gyr}_w$、加速度计噪声密度 $\mathrm{acc}_n$ 以及加速度计偏置随机游走 $\mathrm{acc}_w$，并形成系统运行所需的惯性噪声模型参数。这些参数将在 VIO 系统中用于 IMU 预积分协方差传播、状态预测噪声建模以及后端优化权重配置，是保障真实平台定位精度与系统稳定性的关键基础。
