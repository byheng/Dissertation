...
\begin{minipage}[b]{0.32\linewidth}
  \centering
  \includegraphics[width=\linewidth]{Img/MH_01_easy_traj_trajectories.png}
  \small MH\_01\_easy (ATE: 0.042 m)
\end{minipage}\hfill
\begin{minipage}[b]{0.32\linewidth}
  \centering
  \includegraphics[width=\linewidth]{Img/V1_01_easy_traj_trajectories.png}
  \small V1\_01\_easy (ATE: 0.035 m)
\end{minipage}\hfill
\begin{minipage}[b]{0.32\linewidth}
  \centering
  \includegraphics[width=\linewidth]{Img/MH_04_difficult_traj_trajectories.png}
  \small MH\_04\_difficult (ATE: 0.113 m)
\end{minipage}
\bicaption{EuRoC 数据集代表性序列轨迹对比（蓝色为估计轨迹，橙色为真值）}{Trajectory comparison on representative EuRoC sequences (blue: estimated, orange: ground truth)}
...
从表~\ref{tab:trajectory_accuracy} 可以看出，FPGA 加速器在 EuRoC 数据集各测试序列上的轨迹精度与纯软件基准基本一致（平均 ATE 分别为 0.073 m 与 0.075 m），误差差异处于数值噪声范围内。图~\ref{fig:euroc_trajectories} 展示了三条代表性序列（MH\_01\_easy、V1\_01\_easy 和 MH\_04\_difficult）的估计轨迹与真值轨迹对比，可以看出，在易、中、难不同难度序列下，系统均能保持与真值高度吻合的轨迹，即便在 MH\_04\_difficult 等剧烈运动场景下轨迹估计依然稳定，验证了软硬协同设计在精度上的等效性。同样，表~\ref{tab:ate_tumvi} 在 TUM-VI 数据集上的结果显示，软硬协同加速方案的平均 ATE 为 0.152 m，优于纯软件基准的 0.182 m，且在多数序列上表现更优。图~\ref{fig:tumvi_corridor} 展示了系统在 TUM-VI 走廊序列下的实时建图与定位轨迹结果，可以看到，即便在长距离走廊这种具有挑战性的环境下，系统依然能够稳定构建精确的特征地图并保持平滑的轨迹估计。这些结果表明，硬件加速器在显著提高计算效率的同时，不仅未牺牲定位精度，在部分场景下甚至因数值稳定性的改善而略有提升。
...
\begin{figure}[h!t]
\centering
\includegraphics[width=0.9\linewidth]{Img/outdoors.pdf}
...
在计算效率方面，Schur Kernel 硬件实现将单次后端优化延迟从 $2.84\,\mathrm{ms}$ 压缩至 $0.76\,\mathrm{ms}$，实现 $\mathbf{3.74\times}$ 的加速比，系统后端优化频率稳定在 $\mathbf{25\,\mathrm{Hz}}$ 以上，满足实时 VIO 的运行要求。在资源利用率方面，本文方案 BRAM 占用仅为 $\mathbf{67\,\mathrm{KB}}$，显著低于同类 FPGA 后端加速方案（最低对比为 $230\,\mathrm{KB}$，最高达 $5787\,\mathrm{KB}$），这得益于 SOB 编码对 Hessian 稀疏子块存储的针对性优化。在功耗与能效方面，FPGA 方案动态功耗为 $6.80\,\mathrm{W}$，单帧能耗为 $\mathbf{19.1\,\mathrm{mJ}}$，优于所有参照方案。
