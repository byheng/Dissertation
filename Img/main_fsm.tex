...
\begin{tikzpicture}[
    ...
    state/.style={draw, thick, rounded corners=5pt, minimum width=2.5cm, minimum height=0.9cm, align=center, font=\footnotesize},
...
\node[state_load] (load_param) at (0, -1.8) {LOAD\_PARAMS\\读取state\_param};

\node[state_load] (load_input) at (0, -3.6) {LOAD\_INPUTS\\并行读取SOB/pts/uv};
...
\node[state_phase2] (p2_check) at (4.5, -6.8) {检查 SOB 编码\\(state, cam)有效?};
...
\node[state_phase3] (v_inv) at (2.5, -12) {$\mathbf{V}^{-1}$计算\\(j==0时)};
\node[state_phase3] (wvinv) at (6.5, -12) {$\mathbf{W}\mathbf{V}^{-1}$\\乘法};
\node[state_phase3] (schur_update) at (4.5, -13.5) {Schur更新\\$\mathbf{A}, \mathbf{b}$};
